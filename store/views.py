...
def store(request, category_slug=None):
    ...
    Si es una petición HTMX, devuelve solo el bloque de productos (sin layout completo).
    """
    categories = None
    products = None
...
def search(request):
    ...
    Si es una petición HTMX (submit del formulario), devuelve solo el bloque de productos.
    """
    products = []
    product_count = 0
...
def search_preview(request):
    """
    Endpoint exclusivo para el dropdown de búsqueda en vivo del navbar (HTMX).
    Devuelve hasta 6 resultados en formato compacto para el dropdown.
    """
    products = []
    ...
    if keyword:
        products = Product.objects.filter(
            ...
            is_available=True,
        ).order_by("product_name")[:6]

    context = {
        "products": products,
        "keyword": keyword,
    }
    return render(request, "store/partials/search_results.html", context)
